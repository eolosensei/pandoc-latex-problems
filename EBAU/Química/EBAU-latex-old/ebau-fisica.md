...
Indique un valor aceptable para el número cuántico cuyo valor falta en
...
Prediga, utilizando los potenciales estándar de reducción, los
...
2.  Transferir la mezcla en equilibrio a un recipiente con un volumen
...
Para la reacción en equilibrio a 673 K:
...
2.  A continuación se añaden, en el mismo tubo, 5 mL de un disolvente
...
Escriba la ecuación química ajustada que representa la obtención de
...
Indique y describa el tipo de reacción química que representa cada una
...
El permanganato de potasio, , reacciona con el cloro, , en presencia de
...
1.  Calcule la molaridad inicial de una disolución acuosa de cuyo pH es
    11.5. (1,75 puntos)
...
Se desea realizar la electrolisis de 255 mL de una disolución acuosa
...
2.  Calcule la molaridad de la disolución acuosa de dicromato de
    potasio. (1,0 punto)
...
Escriba las configuraciones electrónicas de los iones
...
Indique, de forma razonada, las condiciones de temperatura (alta o baja)
...
1.  Escriba la ecuación química que representa el equilibrio entre los
...
2.  Calcule los gramos de que quedan en los 500 mL de disolución después
...
A partir de las siguientes configuraciones electrónicas:
...
Escriba las configuraciones electrónicas de los átomos e iones: , , e ,
que ocupan las posiciones de la tabla periódica que se indican a
continuación: X: período = 3 , grupo = 16; Y: período = 4, grupo = 2.

El es 3000 veces más soluble en agua que el fosfano, . Explique la
diferencia en las solubilidades de las dos sustancias en agua si ambas
presentan una geometría molecular de pirámide trigonal.
...
disolución acuosa de hidróxido de sodio, , con una disolución acuosa de
ácido clorhídrico, .

Nombre el grupo funcional presente en cada uno de los siguientes
compuestos:
...
Se añaden 0.5 g de hidróxido de sodio sólido, , a 400 mL de una
disolución acuosa de ácido clorhídrico, , cuyo $pH = 2$. Calcule el pH
de la disolución resultante. Suponga que no se produce variación de
volumen al añadir el sólido a la disolución acuosa.

*Datos:*

Masas atómicas: $\ch{Na} = \SI{23}{\amu}$ ; $\ch{O} = \SI{16}{\amu}$ ;
$\ch{H} = \SI{1}{\amu}$ .

En un recipiente, en el que previamente se ha realizado el vacío, se
introducen 2.0 mol de pentacloruro de fósforo, , y se calienta hasta
450 K, alcanzándose el equilibrio: $$\ch{ PCl5(g) <> PCl3(g) + Cl2(g)}$$
En el equilibrio, la presión total de la mezcla gaseosa es 1 y el se
encuentra disociado en un 36%. Calcule los valores de $K_p$ y $K_c$ para
el equilibrio a 450 K.
...
Para la subcapa electrónica que presenta los valores de los números
cuánticos $n = 5$ y $l = 1$, indique:

1.  la notación de la subcapa;

2.  los valores posibles de $m_l$;

3.  el número de orbitales en la subcapa;

4.  el número máximo de electrones en la subcapa.

Justifique todas las respuestas.

Calcule la variación de energía libre estándar para la formación de a
partir de y a 25 °C, si $\Delta H^0_R = \SI{180.7}{\kJ}$ y
$\Delta S^0_R = \SI{24.7}{\J\per\K}$. Indique si la reacción es
espontánea es esas condiciones. Justifique la respuesta.

Complete y ajuste, por el método del ion-electrón, la siguiente ecuación
química:
$$\ch{NO2^-(ac) + Cr2O7^{2-}(ac) -> Cr^{3+}(ac) + NO3^-(ac)} \quad \text{en medio ácido}$$
...
El denominado *gas de agua* tiene una composición en volumen de 55% de ,
33% de y 12% de . Este gas puede utilizarse como combustible,
produciéndose simultáneamente las reacciones: $$\begin{aligned}
    \ch{CO(g) + 1/2 O2(g) -> CO2(g) \\
        H2(g) + 1/2 O2(g) -> H2O(l)    }
  \end{aligned}$$ El no es combustible. Calcule el calor liberado en la
combustión, a presión constante, de 1 L de gas de agua medido en
condiciones normales de presión y temperatura. Suponga comportamiento
ideal.
...
$\Delta H^\circ_\text{f}[\ch{CO(g)}] = \SI{-110,5}{\kJ\per\mol}$ ;
$\Delta H^\circ_\text{f}[\ch{CO2(g)}] = \SI{-393,5}{\kJ\per\mol}$ ;
$\Delta H^\circ_\text{f}[\ch{H2O(l)}] = \SI{-285,8}{\kJ\per\mol}$ .
$R = \SI{0,082}{\atm\L\per\mol\per\K}$ .

Calcule la masa, en gramos, de cianuro de hidrógeno gaseoso, , que se
necesita para preparar 300 mL de una disolución acuosa de ácido
cianhídrico, , de $pH = 4,5$.

*Datos:*

Masas atómicas: $\ch{C} = \SI{12}{\amu}$ ; $\ch{N} = \SI{14}{\amu}$ ;
$\ch{H} = \SI{1}{\amu}$ . $K_a(\ch{HCN}) = \num{6,2e-10}$

Un erlenmeyer provisto de un cierre hermético contiene una mezcla en
equilibrio a 25 °C de (gas marrón-amarillento) y de (gas incoloro). Se
observa que al calentar el erlenmeyer a 80 °C el color de la mezcla
cambia a marrón intenso, mientras que cuando se enfría a  °C, la mezcla
se vuelve casi incolora.

1.  Escriba la ecuación química que representa el equilibrio entre los
    dos óxidos de nitrógeno. (0,25 puntos)

2.  Indique y justifique el carácter endotérmico o exotérmico de la
    reacción de descomposición del para dar . (0,75 puntos)

A partir de la configuración electrónica del catión :
$1\text{s}^{2}2\text{s}^{2}2\text{p}^{6}3\text{s}^{2}3\text{p}^{6}3\text{d}^{10}4\text{s}^{2}$
, indique la configuración electrónica del elemento X, su número atómico
y el grupo y período de la tabla periódica a los que pertenece.

Deduzca el carácter polar, o no polar, de las siguientes moléculas:

1.  (geometría molecular angular);

2.  (geometría molecular tetraédrica).

En medio acuoso ácido, el ion sulfito, , reacciona con el ion
permanganato, , para formar ion sulfato, , y catión . Ajuste en forma
iónica, mediante el método del ion-electrón, la ecuación química que
representa la reacción indicada.

Nombre y escriba las fórmulas semidesarrolladas de los compuestos
químicos orgánicos que se forman en las oxidaciones sucesivas del etanol
hasta la obtención del ácido correspondiente.

En un recipiente cerrado de 0.5 L, en el que previamente se ha realizado
el vacío, se introducen 1.0 g de y 1.06 g de . Se eleva la temperatura
de la mezcla hasta 1670 K, alcanzándose el equilibrio:
$$\ch{2 H2S(g) <> 2 H2(g) + S2(g)}$$ En el equilibrio, la fracción molar
de en la mezcla gaseosa es 0.015. Calcule el valor de $K_p$ para el
equilibrio a 1670 K.

*Datos:*

Masas atómicas: $\ch{H} = \SI{1}{\amu}$ ; $\ch{S} = \SI{32}{\amu}$ .
$R = \SI{0,082}{\atm\L\per\mol\per\K}$

Cuando se realiza la electrolisis de una disolución acuosa de un cloruro
de rutenio, mediante el paso de una corriente de 0.12 A durante 500 s,
se depositaron 31.0 mg de rutenio metálico en el cátodo.

1.  Dibuje el esquema de la célula electrolítica utilizada en la
    electrolisis con electrodos inertes de platino. Indique el polo
    negativo, el polo positivo del dispositivo y el flujo de electrones
    durante la electrolisis. (1,0 punto)

2.  Determine el número de oxidación del rutenio en el cloruro de
    rutenio de la disolución y escriba la reacción que tiene lugar en el
    cátodo. (1,5 puntos)

*Datos:*

Constante de Faraday $\si{\F} = \SI{96485}{\coulomb\per\mol}$ de
electrones. Masa atómica del $\ch{Ru} = \SI{101,0}{\amu}$ .

Dibuje un esquema del dispositivo experimental necesario para realizar
una valoración ácido-base, indicando el nombre del material de
laboratorio utilizado.

Indique, de forma razonada, el número y tipo de orbitales en un átomo
que presentan el valor del número cuántico principal $n = 3$.

Las energías de red del y del son , respectivamente. Justifique la
diferencia entre estos valores de las energías de red, si ambos
compuestos presentan el mismo tipo de estructura cristalina. Indique, de
forma razonada, el compuesto que, previsiblemente, será más soluble en
agua.

A partir de las entalpías de reacción estándar:

$\Delta H^\circ = \SI[retain-explicit-plus]{-296.83}{\kJ}$

$\Delta H^\circ = \SI[retain-explicit-plus]{-791.44}{\kJ}$

Calcule la entalpía, en condiciones estándar, de la reacción:
$$\ch{2 SO2(g) + O2(g) -> 2 SO3(g)}$$

Escriba las fórmulas semidesarrolladas y nombre los isómeros geométricos
del compuesto 1,2-dicloro-propeno.

Construya el ciclo de Born-Haber para la formación del , a partir de
yodo sólido y sodio metálico, y calcule la energía de red
($\Delta H_{red}$) del compuesto, a partir de los siguientes datos:

-   Entalpía estándar de formación del
    $\Delta H_\text{f}[\ch{NaI}] = \SI{-287,8}{\kJ\per\mol}$

-   Entalpía de sublimación del sodio metálico
    $\Delta H_\text{S}[\ch{Na(s)}] = \SI{107,3}{\kJ\per\mol}$

-   Entalpía de sublimación del yodo sólido
    $\Delta H_\text{S}[\ch{I2(s)}] = \SI{62,44}{\kJ\per\mol}$

-   Entalpía de disociación del
    $\Delta H_\text{D}[\ch{I2(g)}] = \SI{151}{\kJ\per\mol}$

-   Primera energía de ionización del
    $\Delta H_\text{ioniz}[\ch{Na(g)]}] = \SI{495,8}{\kJ\per\mol}$

-   Afinidad electrónica del
    $\Delta H_\text{afin}[\ch{I(g)}] = \SI{-295,2}{\kJ\per\mol}$

El oro se suele depositar sobre otros metales menos caros mediante
electrolisis, dando lugar a la joyería de objetos bañados en oro. La
reacción electrolítica en la superficie donde se deposita el oro es:
$$\ch{Au^{3+}(ac) + 3 e^- -> Au(s)}$$ En la celda electrolítica, el
objeto que se baña con oro es uno de los electrodos y el otro es una
lámina de oro.

1.  Indique el electrodo que actúa como ánodo y el que actúa como cátodo
    en la celda. Escriba las reacciones que tienen lugar en el ánodo y
    en el cátodo de la célula electrolítica. Justifique las respuestas.
    (1,5 puntos)

2.  Cuando se hace pasar una corriente de 2.5 A durante 7.5 min, se
    depositan 0.65 g de oro. Calcule el rendimiento del proceso
    electrolítico. (1,0 punto)

*Datos:*

Masa atómica del $\ch{Au} = \SI{197,0}{\amu}$ ;
$\SI{1}{\F} = \SI{96485}{\coulomb\per\mol}$ de electrones.
...
Ordene las siguientes especies de acuerdo con el valor creciente de su
radio: , , . Justifique la respuesta.

Para el anión , deduzca la estructura de Lewis. Nombre y dibuje la
geometría molecular e indique los ángulos de enlace aproximados.
...
Calcule los moles de carbonato de calcio sólido, , que se obtienen al
evaporar a sequedad 100 mL de una disolución acuosa saturada de
carbonato de calcio.
...
Determine si se formará precipitado cuando se mezclan 250 mL de agua
destilada con 30 mL de disolución acuosa 0.1 M de nitrato de bario, , y
con 20 mL de disolución acuosa de carbonato de sodio, , 0.015 M. Suponga
...
La neutralización exacta de 25 mL de una disolución acuosa de ácido
cianhídrico, , consume 15 mL de disolución acuosa de hidróxido de sodio,
, 0.01 M. Calcule el pH de la disolución acuosa de ácido cianhídrico.
...
1.  $1\text{s}^{2}2\text{s}^{2}3\text{p}^{2}4\text{s}^{1}$ ;

2.  $[\text{Ar}]\ 3\text{d}^{10}4\text{s}^{1}4\text{p}^{3}5\text{s}^{2}$
    .

representan estados excitados de los átomos. Para cada caso escriba la
configuración electrónica del estado fundamental e indique el grupo de
la tabla periódica al que pertenece cada elemento.

Para las sustancias: y , indique, de forma razonada, las fuerzas
intermoleculares presentes en cada una de ellas y la sustancia que
presentará el punto de ebullición más bajo.

Las entalpías estándar de formación de los sólidos y , a partir de y
oxígeno gaseoso, son , respectivamente. A partir de estos datos, calcule
la entalpía estándar de la reacción:
$$\ch{P4O6(s) + 2 O2(g) -> P4O10(s)}$$

Escriba las fórmulas semidesarrolladas de los siguientes compuestos:

1.  2,4,9-trimetildecano
...
Las entalpías estándar de combustión del y del son , respectivamente. La
entalpía estándar de la reacción:

$\Delta H^\circ = \SI[retain-explicit-plus]{-108}{\kJ\per\mol}$

1.  Calcule la entalpía estándar de formación del . (1,75 puntos)

2.  Si desea favorecer la formación de , a partir de y , utilizaría
    temperaturas ¿altas o bajas? Justifique la respuesta. (0,75 puntos)

A 523 K la constante de equilibrio para la reacción:
$$\ch{PCl5(g) <> PCl3(g) + Cl2(g)}$$ tiene el valor
$K_c = \num{3,8e-2}$. En un recipiente de 2.5 L, en el que previamente
se ha realizado el vacío, se introducen 0.50 mol de , 0.25 mol de y
0.25 mol de . La mezcla gaseosa se calienta a la temperatura de 523 K.

1.  Indique el sentido en que debe evolucionar el sistema para alcanzar
    el equilibrio. (1,0 punto)

2.  Calcule el número de moles de cada gas en la mezcla una vez
    alcanzado el equilibrio a 523 K. (1,5 puntos)

En un tubo de ensayo se colocan unos cristales de y se añaden 5 mL de
agua ¿Qué observará? Justifique la observación realizada. (0,5 puntos)

A continuación se añaden, en el mismo tubo, 5 mL de un disolvente
orgánico no polar, se agita la mezcla y se deja reposar hasta que se
separen dos fases. Indique y justifique la coloración que presenta cada
una de las fases. (0,5 puntos)

Escriba las configuraciones electrónicas de los elementos
${X}~(Z = \num{17})$ e ${Y}~(Z = \num{35})$ . Indique el grupo y período
de la tabla periódica al que pertenece cada uno de los elementos. A
partir de su posición en la tabla periódica, indique, de forma razonada,
el elemento que presentará el valor más elevado de la primera energía de
ionización.

Indique, justificando la respuesta, el carácter ácido, básico o neutro
de una disolución acuosa de nitrato de potasio ().

Una disolución acuosa ácida contiene las siguientes concentraciones:
$[\ch{MnO4^-}] = \SI{1}{M}$; $[\ch{Cl^-}] = \SI{1}{M}$. Escriba las
semirreacciones ajustadas de oxidación y de reducción que tienen lugar
en la disolución.
...
Se prepara una disolución acuosa por mezcla de 500 mL de disolución
acuosa de , que contiene un 0,3% en masa de y una densidad de
1.1 g/cm^3^, con 150 mL de una disolución acuosa de 0.01 M y con 150 mL
de agua. Calcule el pH de la disolución resultante. Suponga que los
volúmenes son aditivos.
...
La pila que se basa en la reacción química:
$$\ch{Fe(s) + 2 Ag^+(ac) -> Fe^{2+}(ac) + 2 Ag(s)}$$ tiene un potencial
estándar $E^\circ(\ch{pila}) = \SI[retain-explicit-plus]{+1,24}{\V}$ .

1.  Escriba las semirreacciones de oxidación y de reducción que se
    producen en la pila y calcule el valor del potencial estándar de
    reducción del par ().

2.  Dibuje un esquema de la pila, indicando el ánodo, el cátodo y el
    sentido en el que fluyen los electrones. (1,5 puntos)
...
De las configuraciones electrónicas que se dan a continuación, indique
las que corresponden a átomos en su estado fundamental, en estado
excitado y las que son imposibles. Justifique su respuesta.
...
Los valores de las energías de red para los compuestos y son ,
respectivamente. Suponiendo que ambos compuestos presentan el mismo tipo
de estructura cristalina, explique las diferencias observadas entre los
valores de las energías de red de los dos compuestos.
...
Considere la reacción en equilibrio:
$$\ch{4 NH3(g) + 5 O2(g) <> 4 NO(g) + 6 H2O(g)}$$ Explique cómo afecta
al rendimiento de en el equilibrio una disminución del volumen del
...
-   Electrodo estándar en medio ácido clorhídrico. Las disoluciones del
    electrodo son de permanganato de potasio, , y de cloruro de
    manganeso(II), .

-   Electrodo estándar de . La disolución del electrodo es de cloruro de
    potasio, .

Con los datos de la pila:

1.  Escriba la ecuación química ajustada, en forma molecular, de la
    reacción química que tiene lugar durante el funcionamiento de la
    pila. (1,5 puntos)

2.  Indique el electrodo que actuará como ánodo y el que actuará como
    cátodo. Calcule el potencial estándar de la pila. Justifique la
    respuesta. (1,0 punto)

*Datos:*

$E^\circ(\ch{MnO4^-/Mn^{2+}}) = \SI[retain-explicit-plus]{+1,51}{\V}$ .
$E^\circ(\ch{Cl2/Cl^-}) = \SI[retain-explicit-plus]{+1,359}{\V}$ .

A partir de los valores de las entalpías de reacción:

-   $\qquad \Delta H_a = \SI{-537}{\kJ}$

-   $\qquad \Delta H_b = \SI{-680}{\kJ}$

-   $\qquad \Delta H_c = \SI{+523}{\kJ}$

1.  Calcule la entalpía de la reacción de etileno, , con flúor, , para
    formar tetrafluoruro de carbono, , y fluoruro de hidrógeno, . (1,5
    puntos)

2.  Calcule el calor desprendido en la reacción de 2 g de con 12 g de ,
    a presión constante. (1,0 punto)
...
En un tubo de ensayo se vierten 5 L de una disolución acuosa de
carbonato de sodio () y a continuación se añade, gota a gota, una
disolución acuosa de cloruro de bario, (), hasta la formación de un
precipitado claramente visible. Escriba la fórmula química del compuesto
que precipita. Indique el reactivo que utilizaría para disolver el
precipitado formado. Justifique la respuesta.

Los elementos X e Y ocupan las posiciones de la tabla periódica que se
indican a continuación: X (período = 4 , grupo = 17); Y (período = 4,
grupo = 1).

1.  Escriba las configuraciones electrónicas de los átomos de los dos
    elementos en estado fundamental. (0,5 puntos)

2.  Indique el carácter, iónico o covalente, del enlace presente en el
    compuesto formado por los dos elementos. Justifique la respuesta.
    (0,5 puntos)

El y el son dos compuestos moleculares que presentan geometría molecular
angular. Indique el compuesto que presenta el valor más elevado del
punto normal de ebullición. Justifique la respuesta.
...
Indique, de forma razonada, el carácter ácido, básico o neutro de la
disolución acuosa resultante de la neutralización exacta de una
disolución acuosa de hidróxido de potasio, , con una disolución acuosa
de ácido acético, .
...
Se añaden 50 mL de disolución acuosa de hidróxido de sodio, , de
$pH = 12$, a 30 mL de disolución acuosa de ácido clorhídrico, , de
$pH = 1,5$. Calcule el pH de la disolución resultante. Suponga que los
volúmenes son aditivos.

En el proceso de fabricación del ácido sulfúrico es importante el
equilibrio que se establece en la reacción del con el para formar . En
un recipiente de 2 L, en el que previamente se ha realizado el vacío, se
introducen 0.15 mol de y 0.15 mol de . La mezcla se calienta hasta
900 K. Cuando se alcanza el equilibrio, se han formado 0.092 mol de .
Calcule el valor de $K_p$ para el equilibrio a 900 K.

*Datos:*

$R = \SI{0,082}{\atm\L\per\mol\per\K}$

En dos tubos de ensayo se colocan unos cristales de . En el tubo 1 se
añaden 5 L de agua y en el tubo 2 se añaden 5 L de un disolvente
orgánico no polar. Indique y justifique las observaciones realizadas en
cada uno de los tubos de ensayo.

Para el subnivel electrónico caracterizado por los valores de los
números cuánticos $n = 4$ y $l = 2$, indique:

1.  la notación del subnivel;
...
Para la reacción de síntesis del metanol a 25 °C:
$$\ch{CO(g) + 2 H2(g) -> CH3OH(g)}$$ $\Delta G^0_R = \SI{-24.8}{\kJ}$ y
$\Delta S^0_R = \SI{-219,3}{\J\per\K}$. Calcule el valor de
$\Delta H^0_R$ e indique si la reacción es endotérmica o exotérmica.
Justifique la respuesta.

Complete y ajuste, por el método del ion-electrón, la siguiente ecuación
química: $$\ch{KMnO4(ac) + KI(ac) + H2SO4(ac) -> MnSO4(ac) + I2(s)}$$

Escriba las fórmulas semidesarrolladas de los siguientes compuestos:

1.  2-cloro-4-metil-octano

2.  2,6-dimetil-2-hepteno

3.  Dipropil éter

4.  3-pentanona

La trinitroglicerina, , conocida commente como nitroglicerina, es un
líquido que se utiliza como explosivo. La entalpía estándar de
descomposición de la trinitroglicerina, para formar nitrógeno gas,
dióxido de carbono gas, oxígeno gas y agua líquida, es  kJ/mol de
trinitroglicerina.

1.  Escriba la ecuación química ajustada para la descomposición de la
    trinitroglicerina y calcule el volumen que ocuparán los gases
    resultantes de la descomposición de 1 mol de trinitroglicerina
    líquida a 1 y 25 °C. (1,25 puntos)

2.  Calcule la entalpía estándar de formación de la trinitroglicerina.
    (1,25 puntos)
...
En el laboratorio se obtienen pequeñas cantidades de cloro gas
adicionando gota a gota una disolución acuosa de ácido clorhídrico, ,
sobre cristales de permanganato de potasio, . En la reacción también se
obtiene :

1.  Escriba y ajuste, por el método del ion-electrón, en forma iónica y
    molecular, la reacción química que tiene lugar. Indique la especie
    que actúa como reductor. (1,75 puntos)

2.  Calcule la mínima cantidad, en gramos, de permanganato de potasio
    necesaria para obtener 1 L de cloro gas a 25 °C y 1. Suponga que el
    rendimiento de la reacción es del 100%. (0,75 puntos)

*Datos:*

Masas atómicas: $\ch{Mn} = \SI{54,94}{\amu}$ ; $\ch{O} = \SI{16}{\amu}$
; $\ch{K} = \SI{39,1}{\amu}$ . $R = \SI{0,082}{\atm\L\per\mol\per\K}$

Un tubo de ensayo contiene 5 mL de una disolución acuosa de nitrato de
plata, , a la que se añade, gota a gota, una disolución acuosa de
cloruro de sodio, , hasta la formación de un precipitado claramente
visible. Escriba la fórmula química del compuesto que precipita. ¿Qué
reactivo utilizaría para disolver el precipitado formado? Justifique la
respuesta.

Escriba las configuraciones electrónicas de los elementos
${X}~(Z = \num{16})$ e ${Y}~(Z = \num{52})$ . Indique el grupo y período
al que pertenece cada uno de los elementos. Indique, de forma razonada,
el elemento que presenta el valor más elevado del radio atómico.

Deduzca, de forma razonada, el carácter polar, o no polar, de la
molécula .
...
Para preparar una disolución acuosa básica utilizaría como soluto:
...
Nombre y escriba las fórmulas semidesarrolladas de los compuestos
orgánicos obtenidos en la adición de agua a 1-buteno ().

En un matraz de 5.0 L, en el que inicialmente se ha realizado el vacío,
se introducen 0.1 mol de cada uno de los siguientes gases: , , e . Se
eleva la temperatura del recipiente hasta 1000 K, alcanzándose el
equilibrio a 1000 K:

$K_\text{c} = \num{7,4e-3}$

1.  Justifique si la mezcla gaseosa inicial se encuentra en equilibrio,
    o no, a 1000 K, y el sentido en el que evolucionará el sistema para
    alcanzar el equilibrio. (1,0 punto)

2.  Si la presión total de la mezcla gaseosa en el equilibrio a 1000 K
    es de 7, calcule su composición, en moles de cada gas. (1,5 puntos)

*Datos:*

$R = \SI{0,082}{\atm\L\per\mol\per\K}$

Se mezclan 25.0 mL de una disolución acuosa de ácido nítrico, , de
$pH = 2,12$, con 25.0 mL de una disolución acuosa de hidróxido de
potasio, , de $pH = 12,6$. Calcule el pH de la disolución acuosa
resultante. Suponga que los volúmenes son aditivos.

En dos tubos de ensayo se colocan unos cristales de permanganato de
potasio, . En el tubo 1 se añaden 5 mL de agua y en el tubo 2 se añaden
5 mL de un disolvente orgánico no polar. Indique y justifique las
observaciones realizadas en cada uno de los tubos de ensayo.
...
Para una determinada reacción química $\Delta H = \SI{17,8}{\kJ}$ y
$\Delta S = \SI{123}{\J\per\K}$. Calcule la temperatura a partir de la
cual la reacción será espontánea.

La pila voltaica representada por el esquema:
$$\ch{Cd(s)}\,|\,\ch{Cd^{2+}(ac,~1~M)}\,||\,\ch{Cu^{2+}(ac,~1~M)}\,|\,\ch{Cu(s)}$$

tiene un potencial estándar de pila de 0.743 V. Escriba la ecuación
química que representa la reacción que tiene lugar en el ánodo y calcule
el potencial estándar de reducción del electrodo .
...
1.  1,1,1-clorodifluoroetano

2.  2-cloro-3-metil-2-penteno
...
Construya el ciclo de Born-Haber para la formación del , a partir de
bromo líquido y sodio metálico, y calcule la energía de red
($\Delta H_{red}$) del compuesto, a partir de los siguientes datos:

-   Entalpía estándar de formación del
    $\Delta H_\text{f}[\ch{NaBr}] = \SI{-361,4}{\kJ\per\mol}$ ;
...
En el proceso de purificación del mediante electrolisis, una lámina de
impuro actúa como ánodo de la célula electrolítica y una lámina de de
elevada pureza actúa como cátodo. Ambas están sumergidas en una
disolución ácida de .

1.  Dibuje un esquema de la célula electrolítica, indicando el polo
    positivo, el polo negativo y el flujo de electrones durante el
    proceso de electrolisis. Escriba las reacciones que se producen en
    el ánodo y en el cátodo. (1,5 puntos)

2.  Calcule el tiempo que tiene que estar funcionando la célula para que
    la masa del cátodo aumente en 1.5 g al pasar una corriente de 5 A.
    (1,0 punto)

*Datos:*

Constante de Faraday $\si{\F} = \SI{96485}{\coulomb\per\mol}$ de
electrones. Masa atómica del $\ch{cobre} = \SI{63,55}{\amu}$ .

r0.18 ![image](bureta-erlenmeyer){width="18%"}

En el laboratorio se dispone del dispositivo experimental de la figura y
del material de laboratorio y reactivos que se relacionan: pipeta
aforada de 10 mL, disolución acuosa titulada de , muestra de vinagre
comercial e indicador. Indique el procedimiento experimental a seguir
para realizar la determinación del contenido de ácido acético en un
vinagre comercial.

Justifique la siguiente relación de radios:
$r(\ch{O^{2-}}) > r(\ch{O}) > r(\ch{F})$.

*Datos:*

${O}~(Z = \num{8})$ ; ${F}~(Z = \num{9})$

Deduzca la estructura de Lewis para el metanal, . Nombre y dibuje su
geometría molecular e indique los ángulos de enlace aproximados.
...
Calcule el número de moles de disueltos en 100 mL de una disolución
acuosa saturada de la sal a 25 °C.
...
Escriba las fórmulas semidesarrolladas y nombre los isómeros geométricos
del 1,2-dicloro-1-buteno.

Determine si se formará precipitado cuando a 100 mL de agua destilada se
añaden 100 mL de disolución acuosa 0.01 M de sulfato de sodio, , y 10 mg
de nitrato de plomo(II), , sólido. Suponga que los volúmenes son
aditivos y que el volumen del sólido puede despreciarse.

*Datos:*

$K_{PS}(\ch{PbSO4}) = \num{1,6e-8}$ . Masas atómicas:
$\ch{Pb} = \SI{207,2}{\amu}$ ; $\ch{N} = \SI{14}{\amu}$ ;
$\ch{O} = \SI{16}{\amu}$ .

Calcule el pH de la disolución que se obtiene al añadir a 20 mL de agua
destilada, 20 mL de disolución acuosa 0.01 M de hidróxido de bario, , y
20 mL de HCl(ac) 0.01 M. Suponga que los volúmenes son aditivos.
...
1.  $1\text{s}^{2}2\text{s}^{2}2\text{p}^{6}3\text{s}^{2}3\text{p}^{6}4\text{s}^{1}3\text{d}^{8}4\text{p}^{1}5\text{s}^{1}4\text{d}^{4}$

2.  $1\text{s}^{2}2\text{s}^{2}2\text{p}^{6}3\text{s}^{2}3\text{p}^{6}4\text{s}^{2}3\text{d}^{10}4\text{p}^{6}4\text{d}^{3}$

representan estados excitados de los átomos. Para cada caso, escriba la
configuración electrónica del estado fundamental e indique el período de
la tabla periódica al que pertenece cada elemento.

Justifique las variaciones observadas en los valores de las temperaturas
de ebullición de las siguientes sustancias a 1:

  Compuesto                                    
  -------------------------------- ----- ----- -----
  Temperatura de ebullición (°C)    -85   -67   -35

Determine el intervalo de temperaturas (altas o bajas) en el que la
siguiente reacción es espontánea:
$$\ch{NO(g) + 1/2 Cl2(g) -> NOCl(g)} \qquad \Delta H_R = \SI{-38,54}{\kJ}$$

Escriba las fórmulas semidesarrolladas de los siguientes compuestos:

1.  1,3,5-tribromo-2-penteno
...
A partir de los siguientes valores de las entalpías estándar de
formación:
$\Delta H^\circ_\text{f}[\ch{HF(g)}] = \SI{-268,5}{\kJ\per\mol}$ ;
$\Delta H^\circ_\text{f}[\ch{CF4(g)}] = \SI{-680,0}{\kJ\per\mol}$ y
$\Delta H^\circ_\text{f}[\ch{C2H4(g)}] = \SI{+52,3}{\kJ\per\mol}$ :

1.  Calcule la variación de entalpía estándar para la reacción de
    etileno, , con flúor, , para formar tetrafluoruro de carbono, , y
    fluoruro de hidrógeno, . (1,5 puntos)

2.  Si desea evitar la formación de tetrafluoruro de carbono, utilizaría
    temperaturas ¿altas o bajas? Justifique la respuesta. (1,0 punto)

Para la reacción química en equilibrio , el valor de $K_c$ a 1000 K es
1.9. En un recipiente de 3 L, en el que inicialmente se ha realizado el
vacío, se introduce un exceso de carbono y 25.0 g de . La temperatura
del recipiente se eleva hasta 1000 K.

1.  Calcule la masa, en gramos, de que se produce en el recipiente y los
    gramos de carbono que se consumen a 1000 K. (1,75 puntos)

2.  Calcule el valor de la constante $K_p$ para la reacción en
    equilibrio a 1000 K. (0,75 puntos)
...
Indique el grupo de la tabla periódica al que pertenece cada uno de los
elementos representados por las siguientes configuraciones electrónicas
generales. Justifique la respuesta.

1.  $[\text{gas noble}]\ n\text{s}^{2}n\text{p}^{5}$

2.  $[\text{gas noble}]\ n\text{s}^{1}$

3.  $[\text{gas noble}]\ n\text{s}^{2}(n-1)\text{d}^{10}n\text{p}^{1}$

4.  $[\text{gas noble}]\ n\text{s}^{2}(n-1)\text{d}^{2}$

Se prepara una disolución reguladora de ácido acético, , y de acetato de
sodio, . Explique, de forma cualitativa, el funcionamiento de esta
disolución en el control del pH cuando se añaden unas gotas de ácido
nítrico a la disolución. Escriba la ecuación iónica de la reacción que
se produce al añadir el ácido nítrico a la disolución reguladora.

Para la molécula , deduzca la estructura de Lewis. Nombre y dibuje la
geometría molecular alrededor de los átomos de carbono y de oxígeno.

*Datos:*

${C}~(Z = \num{6})$ , ${H}~(Z = \num{1})$ , ${O}~(Z = \num{8})$ .

Escriba las fórmulas semidesarrolladas correspondientes a cada uno de
los siguientes compuestos:

1.  4-etil-2,3-dimetiloctano

2.  Metilpropilamina

3.  3-pentanona

4.  1,1-dicloro-1-buteno

La neutralización exacta de 20 mL de una disolución acuosa de ácido
acético, (), de concentración desconocida, necesitó 10 L de disolución
acuosa de 0.5 M. Calcule el volumen, en mL, de la disolución inicial de
ácido acético que se necesita para preparar, por dilución con agua,
500 mL de una disolución acuosa de este ácido de $pH = 3,7$.

*Datos:*

$K_a(\ch{CH3COOH}) = \num{1,8e-5}$ .

Las disoluciones acuosas ácidas de dicromato de potasio, , son de color
naranja. Cuando se añade peróxido de hidrógeno, , a una disolución de
dicromato de potasio en medio ácido clorhídrico, , se observa el
desprendimiento gaseoso de oxígeno, un cambio de color en la disolución
y la formación de .

1.  Escriba la ecuación química ajustada, en forma iónica y molecular,
    que representa la reacción química que se produce en la disolución.
    Indique el agente oxidante y el agente reductor. (1,75 puntos)

2.  Calcule el volumen de disolución acuosa 0.25 M de peróxido de
    hidrógeno necesario para que reaccione todo el dicromato de potasio
    contenido en 0.5 L de una disolución acuosa 0.7 M de la sal. (0,75
    puntos)

Dibuje un esquema del dispositivo experimental que permite medir la
conductividad del e indique el material de laboratorio que se utiliza.

Escriba los valores numéricos de los números cuánticos n y l que
corresponden a los electrones que se encuentran en los siguientes
orbitales:
...
Indique el signo, positivo o negativo, de la variación de entropía del
sistema en los siguientes procesos

1.  fusión de un sólido;

2.  licuación de un gas;

3.  evaporación de un líquido;

4.  sublimación de un sólido.

Justifique la respuesta.

Para la reacción química en equilibrio: $\Delta H < 0$, indique y
justifique cómo afectan al valor de las concentraciones de las
sustancias en el equilibrio los siguientes cambios:

1.  disminución del volumen del recipiente a temperatura constante (0,50
    puntos);

2.  aumento de la temperatura manteniendo el volumen constante.(0,50
    puntos)

Los puntos normales de ebullición del metanol (,
$\text{MM} = \SI{32}{\g\per\mol}$) y del etano
(,$\text{MM} = \SI{30}{\g\per\mol}$) son , respectivamente. Justifique
la diferencia entre los dos valores de los puntos normales de
ebullición.

La reacción de metano, , con cloro, , en condiciones estándar, produce
cloruro de hidrógeno, , y cloroformo, . Calcule la variación de entalpía
para la reacción descrita a partir de los siguientes datos:

-   $\Delta H^\circ_\text{f}[\ch{CHCl3(g)}] = \SI{-103,1}{\kJ\per\mol}$
    ;

-   $\Delta H^\circ_\text{f}[\ch{HCl(g)}] = \SI{-92,3}{\kJ\per\mol}$ ;

-   $\Delta H^\circ_\text{f}[\ch{H2O(l)}] = \SI{-285,8}{\kJ\per\mol}$ ;

-   $\Delta H^\circ_\text{f}[\ch{CO2(g)}] = \SI{-393,5}{\kJ\per\mol}$ ;

-   $\Delta H^\circ_\text{comb}[\ch{CH4(g)}] = \SI{-890,3}{\kJ\per\mol}$
    .

Un vinagre comercial contiene un 6,0% en masa de ácido acético, .
Calcule la masa, en gramos, de ese vinagre que debe diluirse en agua
para obtener 500 mL de una disolución con $pH = 3,5$. Considere que el
ácido acético es el único ácido presente en el vinagre.
...
Describa el procedimiento experimental a seguir en el laboratorio para
determinar la concentración de peróxido de hidrógeno en un agua
oxigenada mediante la valoración denominada permanganimetría.

Escriba las configuraciones electrónicas de los elementos
${X}~(Z = \num{17})$ e ${Y}~(Z = \num{53})$ . Indique el grupo y período
de la tabla periódica a los que pertenece cada uno de los elementos. A
partir de su posición en la tabla periódica, indique, de forma razonada,
el elemento que presentará el valor más negativo de la afinidad
electrónica.
...
La reacción de descomposición: $$\ch{NH4HS(s) <> NH3(g) + H2S(g)}$$ es
un proceso endotérmico. Explique el efecto que sobre la concentración de
en el equilibrio tendrá:

1.  Elevar la temperatura de la reacción manteniendo el volumen
    constante. (0,5 puntos)

2.  La adición de al sistema en equilibrio. (0,5 puntos)

Nombre y escriba las fórmulas semidesarrolladas de los productos
obtenidos en:

1.  la oxidación de 2-propanol; (0,5 puntos)

2.  la deshidratación del etanol. (0,5 puntos)

En un recipiente de 5 L, en el que previamente se ha realizado el vacío,
se introducen 10.6 g de y 375.0 g de . El conjunto se calienta a 200 °C,
estableciéndose el equilibrio químico representado por la ecuación:
$$\ch{PCl5(g) <> PCl3(g) + Cl2(g)}$$

1.  Si en el equilibrio hay 30.0 g de , calcule las presiones parciales
    de , y en el equilibrio a 200 °C. (1,5 puntos)

2.  Calcule los valores de $K_p$ y $K_c$ para el equilibrio a 200 °C.
    (1,0 punto)

*Datos:*

$R = \SI{0,082}{\atm\L\per\mol\per\K}$ ; Masas atómicas:
$\ch{P} = \SI{31}{\amu}$ , $\ch{Cl} = \SI{35,45}{\amu}$ .

Se dispone del siguiente material: dos varillas de cobre, disolución 1 M
de , disolución 1 M de , puente salino, voltímetro y conexiones
eléctricas.

1.  Escriba las semirreacciones de oxidación y de reducción, así como la
    reacción global que tiene lugar en la pila que se puede construir
    con el material disponible. Calcule el potencial estándar de la
    pila. (1,0 punto)
...
$E^\circ(\ch{Cu^{2+}/Cu}) = \SI[retain-explicit-plus]{+0,34}{\V}$ ;
$E^\circ(\ch{Cu^+/Cu}) = \SI[retain-explicit-plus]{+0,52}{\V}$ .
...
Indique, justificando la respuesta, el número de electrones desapareados
que presentan, en estado fundamental, los átomos de
${Mn}~(Z = \num{25})$ y ${Se}~(Z = \num{34})$ .

Los valores de los puntos normales de ebullición del y del son ,
respectivamente. Explique la diferencia observada en estos valores de
los puntos normales de ebullición.
...
3.  2,2-dimetil-3-hexino

4.  Butanona

Calcule el pH de la disolución acuosa que se obtiene al añadir a 35 mL
de agua destilada 25 mL de disolución acuosa de (0,5% en masa y
$d = \SI{1,12}{\g\per\mL}$) y 40 mL de disolución acuosa de 0.15 M.

*Datos:*

Masas atómicas: $\ch{Ba} = \SI{137,3}{\amu}$ ; $\ch{O} = \SI{16}{\amu}$
; $\ch{H} = \SI{1}{\amu}$ .

En un recipiente cerrado de 2 L, en el que inicialmente se ha realizado
el vacío, se introducen 0.1 mol de NOCl(g), 0.1 mol de NO(g) y 0.05 mol
de . La mezcla gaseosa se calienta a 300 °C, alcanzándose el equilibrio:
$$\ch{2 NOCl(g) <>  2 NO(g)  + Cl2(g)}$$ En el equilibrio, el número
total de moles gaseosos ha disminuido un 7,2%. Calcule el valor de $K_c$
para la reacción en equilibrio a 300 °C tal y como está escrita.
...
acuerdo con la ecuación química: $$\begin{aligned}
  ...
  \end{aligned}$$ En el laboratorio, 10 mL del agua oxigenada se diluyen
con agua hasta 100 mL y se toma una alícuota de 10 mL. La valoración de
esta alícuota consume, en el punto de equivalencia, 20 mL de una
disolución de permanganato de potasio 0.02 M.
...
2.  indique el nombre del material de laboratorio en el que se coloca la
    disolución acuosa de agua oxigenada durante la valoración (0,25
    puntos).

Escriba las configuraciones electrónicas en estado fundamental de los
elementos ${X}~(Z = \num{19})$ e ${Y}~(Z = \num{36})$ . Indique el grupo
...
bajo de la primera energía de ionización.

Para el anión carbonato, , deduzca la estructura de Lewis. Indique y
dibuje la geometría molecular del anión, según la TRPECV, y los ángulos
...
Escriba el valor de los números cuánticos $n$ y $l$ para los orbitales
de la subcapa 3d.

Escriba las fórmulas semidesarrolladas de los siguientes compuestos:

1.  3,4-dicloro-1-pentino (3,4-dicloropent-1-ino)

2.  Dietilmetilamina

3.  *cis*-2,3-dicloro-2-penteno (*cis*-2,3-dicloropent-2-eno)

4.  Dietil éter

5.  Bromobenceno

6.  3-hexanona (hexan-3-ona)

En una disolución acuosa saturada de carbonato de bario, , la
concentración del anión carbonato es 8.3e-5 M.

1.  Calcule la constante del producto de solubilidad del carbonato de
    bario. (1,0 punto)

2.  Determine si se formará un precipitado de carbonato de bario al
    añadir a 100 mL de agua 30 mL de una disolución acuosa e-3 M de
    nitrato de bario, , y 20 mL de una disolución acuosa e-3 M de
    carbonato de sodio, . (1,5 puntos)

En disolución acuosa ácida, el anión permanganato, , reacciona con el
para formar y anión dicromato, .

1.  Indique, justificando la respuesta, la especie química que se oxida,
    la que se reduce, la que actúa como oxidante y la que actúa como
    reductora. Ajuste la reacción química global en forma iónica
    mediante el método del ion-electrón. (1,25 puntos)

2.  Dibuje un esquema de la célula galvánica basada en la reacción
    química que se produce de forma espontánea, indicando las
    semirreacciones que se producen en el ánodo y en el cátodo de la
    célula y el sentido del flujo de electrones durante su
    funcionamiento. Calcule el potencial estándar de la célula. (1,25
    puntos)
...
El elemento X en estado fundamental presenta la siguiente configuración
electrónica:
...
1.  el grupo y período de la tabla periódica a los que pertenece el
    elemento y su carácter metálico, o no metálico;

2.  el tipo de ion, anión o catión, que formará el elemento.

Justifique las respuestas.

Los puntos de ebullición normales del 1-propanol (propan-1-ol, ) y del
metoxietano (etil metil éter, ) son , respectivamente. Justifique la
diferencia en los valores de los puntos de ebullición normales de los
dos compuestos.

Indique el tipo de hibridación que presenta el átomo de carbono en:

1.  la molécula de (geometría lineal);

2.  la molécula (geometría tetraédrica).

Para la reacción $$\ch{HC+CH + Br2 ->}$$

1.  Nombre y escriba la fórmula semidesarrollada del producto de la
    reacción. (0,5 puntos)

2.  Nombre y escriba la fórmula semidesarrollada de los isómeros
    geométricos del producto de la reacción. (1,0 punto)

Para la reacción química general , a una temperatura determinada, la
velocidad inicial de desaparición de A varía con las concentraciones
iniciales de los reactivos en la forma que se indica en la tabla:

   Exp.   $[A]_0 (\si{M})$   $[B]_0 (\si{M})$   Velocidad inicial (M/s)
  ------ ------------------ ------------------ -------------------------
    1           0,2                0,2                  2,32e-4
    2           0,8                0,2                  9,28e-4
    3           1,2                1,2                  8,35e-3

1.  Determine la ecuación de velocidad para la reacción, indicando el
    orden de reacción parcial respecto del reactivo A y del reactivo B.
    (2,0 puntos)

2.  Calcule el valor de la constante de velocidad, k, e indique sus
    unidades. (0,5 puntos)

Calcule el pH de la disolución resultante de diluir 10 mL de una
disolución acuosa de amoníaco, , al 10% en masa de amoníaco y densidad
0.98 g/mL, con agua hasta un volumen final de la disolución de 1 L.

*Datos:*

$K_b(\ch{NH3}) = \num{1,8e-5}$ . Masas atómicas:
$\ch{N} = \SI{14}{\amu}$ ; $\ch{H} = \SI{1}{\amu}$ .

Indique el material de laboratorio necesario para realizar la
determinación de la concentración de en el agua oxigenada comercial,
utilizando una disolución de permanganato de potasio.

Para el elemento X, caracterizado por pertenecer al grupo 15 y al
período 4 de la tabla periódica:

1.  escriba la configuración electrónica en el estado fundamental;

2.  indique su número atómico;

3.  indique el número de electrones desapareados que presenta en el
    estado fundamental;

4.  escriba la configuración electrónica del anión en estado
    fundamental.

Justifique la diferencia en los valores de las temperaturas normales de
ebullición del (239.8 K) y del (144.1 K), si las dos moléculas presentan
la misma estructura molecular (pirámide trigonal) y las dos son polares.

Deduzca el carácter polar, o no polar, de la molécula , que presenta una
geometría molecular lineal.

Nombre y escriba las fórmulas semidesarrolladas de los compuestos
orgánicos que intervienen en las siguientes reacciones químicas:

1.  Oxidación de 2-propanol (propan-2-ol) con dicromato, , en medio
    ácido. (0,75 puntos)

2.  Deshidratación del etanol en presencia de ácidos fuertes. (0,75
    puntos)

En un recipiente cerrado de 2 L, en el que inicialmente se ha realizado
el vacío, se introducen 0.5 mol de , 0.2 mol de y 0.5 mol de . La mezcla
gaseosa se calienta a 1000 K, alcanzándose el equilibrio representado
por la reacción: $$\ch{2 SO2(g) + O2(g) <> 2 SO3(g)}$$ En el equilibrio,
la presión parcial de es de 10.

1.  Indique, de forma razonada, el sentido en el que evolucionará el
    sistema para alcanzar el equilibrio. (1,0 punto)

2.  Calcule el valor de $K_c$ para la reacción en equilibrio a 1000 K,
    tal y como está escrita. (1,5 puntos)

*Datos:*

$R = \SI{0,082}{\atm\L\per\mol\per\K}$

Se construye una pila voltaica con los siguientes electrodos: a) una
tira de cobre sumergida en una disolución acuosa de 1 M; b) una tira de
plata sumergida en una disolución acuosa de 1 M.

1.  Escriba las semirreacciones de oxidación y de reducción que se
    producen, de forma espontánea, durante el funcionamiento de la pila.
    Calcule el potencial estándar de la pila. (1,0 punto)

2.  Dibuje un esquema de la pila indicando el ánodo, el cátodo y el
    sentido en el que fluyen los electrones cuando funciona la pila.
    (1,5 puntos)
...
En la realización de una volumetría ácido-base para determinar la
concentración de ácido acético, , en una disolución acuosa, 10 mL de la
disolución acuosa del ácido se diluyen con 50 mL de agua. La
neutralización exacta de esta disolución consume 15 mL de disolución
acuosa de hidróxido de sodio, , 0.05 M.

1.  Calcule la concentración del ácido acético en la disolución inicial.
    (0,75 puntos)

2.  Indique el nombre del material de laboratorio en el que se alojaría
    la disolución acuosa de hidróxido de sodio. (0,25 puntos)

Indique el número cuántico, y sus posibles valores, que representa según
la teoría mecanocuántica:

1.  la energía de un orbital;

2.  la orientación espacial de un orbital.

Los elementos X e Y ocupan las posiciones de la tabla periódica que se
indican a continuación: X: periodo = 4, grupo = 13; Y: periodo = 4,
grupo = 17. Indique el elemento que presentará el valor más alto del
radio atómico. Justifique la respuesta.

Indique el tipo de hibridación del átomo central en las siguientes
moléculas:

1.  (geometría tetraédrica);

2.  (geometría lineal).

Identifique y nombre los grupos funcionales presentes en los siguientes
compuestos:
...
Calcule la masa, en gramos, de amoniaco, , que es necesaria para
...
En un recipiente cerrado de 20 L, en el que inicialmente se ha realizado
...
En la disolución preparada disolviendo 9 mg de ácido acético, , en agua
...
-   Entalpía estándar de formación del :
    $\Delta H_\text{f}[\ch{KI}] = \SI{-327,9}{\kJ\per\mol}$ .

-   Entalpía de sublimación del :
    $\Delta H_\text{S}[\ch{K(s)}] = \SI{89,24}{\kJ\per\mol}$ .

-   Entalpía de sublimación del :
    $\Delta H_\text{sublim}[\ch{I2(s)}] = \SI{62,44}{\kJ\per\mol}$ .

-   Entalpía de disociación del :
    $\Delta H_\text{D}[\ch{I2(g)}] = \SI{151}{\kJ\per\mol}$ .

-   Primera energía de ionización del :
    $\Delta H_\text{ionizn}[\ch{K(g)}] = \SI{418,9}{\kJ\per\mol}$ .

-   Afinidad electrónica del :
    $\Delta H_\text{afin}[\ch{I(g)}] = \SI{-295,2}{\kJ\per\mol}$ .

Experimentalmente se determinó que en 250 mL de una disolución acuosa
saturada de carbonato de calcio, , a 25 °C, hay 1.3 mg de sal disueltos.

1.  Calcule el valor de la constante del producto de solubilidad del en
    agua a 25 °C. (2,0 puntos)

2.  Calcule la concentración máxima de que puede estar disuelto en una
    disolución acuosa que presenta una
    $[\ch{CO3^{2-}}] = \SI{1.5e-4}{M}$, a 25 °C. (0,5 puntos)

*Datos:*

Masas atómicas: $\ch{Ca} = \SI{40}{\amu}$ ; $\ch{C} = \SI{12}{\amu}$ ;
$\ch{O} = \SI{16}{\amu}$ .
...
Ajuste, por el método del ion-electrón en medio básico, la siguiente
ecuación química: $$\ch{S(s) + ClO^-(ac) -> SO3^{2-}(g) + Cl^-(g)}$$

Para la reacción química elemental , dibuje:
...
Se mezclan 7.5 mL de una disolución acuosa de ácido nítrico, , de
$pH = 1,5$, con 2.5 mL de una disolución acuosa de ácido clorhídrico, ,
del 0,8% en masa y densidad igual a 1.05 g/mL. La mezcla se diluye con
agua hasta un volumen final de la disolución de 2 L. Calcule el pH de la
disolución resultante.
...
Cuando se añade una disolución acuosa de peróxido de hidrógeno, , a una
disolución acuosa ácida que contiene bromo disuelto, , se produce una
reacción química espontánea. A partir de los valores de los potenciales
estándar de reducción,
$E^\circ(\ch{H2O2/H2O}) = \SI[retain-explicit-plus]{+1,763}{\V}$ ;
$E^\circ(\ch{BrO3-/Br2}) = \SI[retain-explicit-plus]{+1,478}{\V}$ :

1.  Indique, de forma razonada, la especie química en disolución que
    experimenta la reacción de oxidación y la que experimenta la
    reacción de reducción. Escriba y ajuste por el método del
    ion-electrón la ecuación que representa la reacción química que se
    produce de forma espontánea. Indique la especie química que actúa
    como reductor. (2,0 puntos).
...
Los siguientes diagramas de orbitales corresponden a especies químicas
neutras. Indique los diagramas que son correctos, los que son
incorrectos y los que corresponden a estados fundamentales o excitados
del átomo neutro. Justifique todas las respuestas.
...
Escriba las configuraciones electrónicas, en estado fundamental, de los
elementos ${X}~(Z = \num{17})$ e ${Y}~(Z = \num{35})$ . Indique el grupo
y periodo de la tabla periódica a los que pertenece cada uno de los
elementos. A partir de su posición en la tabla periódica, indique, de
forma razonada, el elemento que previsiblemente presentará el valor más
elevado de la primera energía de ionización.

Deduzca, a partir de su estructura molecular, el carácter polar, o no
polar, de la molécula , que presenta una geometría molecular triangular.
...
Escriba las fórmulas semidesarrolladas y nombre tres de los isómeros
posibles del ácido carboxílico con fórmula molecular .

Se prepara una disolución acuosa de ácido cianhídrico, , disolviendo
0.675 g del ácido en un volumen final de disolución de 250 mL. El pH de
la disolución resultante es 5.07. Calcule el valor del grado de
disociación y de la constante de ionización del ácido en la disolución
acuosa a 25 °C.
...
A 250 mL de agua se añade 1 mg de cloruro de plata, AgCl(s), a 25 °C.
Determine:

1.  Si se disolverá todo el sólido añadido. (2,0 puntos)

2.  La \[\] en la disolución. (0,5 puntos).

Suponga que no se observa variación de volumen al añadir el sólido al
agua.

*Datos:*

Masas atómicas: $\ch{Ag} = \SI{107,9}{\amu}$ ;
$\ch{Cl} = \SI{35,45}{\amu}$ . $K_{PS}(\ch{AgCl}) = \num{1,8e-10}$
...
acuerdo con la ecuación química: $$\begin{aligned}
    \ch{2 KMnO4(ac) + 5 H2O2(ac) + 3 H2SO4(ac) -> \\
        ->  2 MnSO4(ac) + 5 O2(g) + 8 H2O + K2SO4(ac)}
  \end{aligned}$$ En el laboratorio, 1 mL del agua oxigenada se diluye
con agua hasta un volumen final de 20 mL. La valoración exacta de esta
disolución consume, en el punto de equivalencia, 15 mL de una disolución
acuosa de permanganato de potasio 0.01 M.

1.  Calcule la concentración de peróxido de hidrógeno en el agua
    oxigenada inicial (0,75 puntos);

2.  Indique el nombre del material de laboratorio que contiene la
    disolución acuosa de peróxido de hidrógeno durante la valoración
    (0,25 puntos).

Para el elemento ${X}~(Z = \num{38})$ , escriba la configuración
electrónica en su estado fundamental e indique, de forma razonada:

1.  el bloque y el período de la tabla periódica a los que pertenece el
    elemento;

2.  el tipo de ion, anión o catión, que formará con mayor facilidad el
    elemento.

Las temperaturas de ebullición a la presión de 1 de las sustancias y
son, respectivamente, 58.8 °C y 97.4 °C. Teniendo en cuenta que las
masas molares de las dos sustancias son muy semejantes
\[$\text{MM}(\ch{Br2}) = \SI{159,8}{\g\per\mol}$,
$\text{MM}(\ch{ICl}) = \SI{162,35}{\g\per\mol}$\], justifique la
diferencia en los valores de las temperaturas de ebullición de estas dos
sustancias.

*Datos:*

Valores de electronegatividad: $\chi (\ch{I}) = \num{2,66}$ ;
$\chi (\ch{Cl}) = \num{3,16}$ .

Para el $\ch{^{238}_{92}U}$ indique, de forma razonada, el número de
protones y de neutrones que hay en el núcleo del átomo.
...
Para la reacción química a 425 °C
...
introducen 13 g de , 2.02 g de y 20.04 g de . La mezcla se calienta a
425 °C.

1.  Indique, de forma razonada, el sentido en el que el sistema
    evolucionará de forma espontánea para alcanzar el estado de
    equilibrio. (1,25 puntos)

2.  Calcule el valor de la concentración en el equilibrio de cada una de
    las sustancias que intervienen en la reacción. (1,25 puntos)

*Datos:*

Masas atómicas: $\ch{I} = \SI{126,91}{\amu}$ ;
$\ch{H} = \SI{1,01}{\amu}$ .

A partir de la notación de la pila galvánica
$$\ch{Ag(s)}\,|\,\ch{Ag^+(ac, 1 M)}\,||\,\ch{Cr2O7^{2-}(ac, 1 M)}\,|\,\ch{Cr^{3+}(ac, 1 M)\,|\,Pt}$$
...
2.  Dibuje un esquema de la pila en el que están representadas la
    semicelda que acta como ánodo y la que actúa como cátodo, así como
    el sentido del flujo de electrones durante el funcionamiento de la
    pila. (1,0 punto)

Para la determinación del contenido en ácido acético de un vinagre
comercial, 10 mL de vinagre se diluyen con agua hasta un volumen final
de 35 mL. La neutralización exacta de esta disolución consume 30 mL de
disolución acuosa de hidróxido de sodio, , 0.1 M.

1.  Calcule la concentración del ácido acético en el vinagre comercial.
    (0,75 puntos)

2.  Indique el nombre del material de laboratorio que contiene la
    disolución acuosa de . (0,25 puntos)

Indique, de forma razonada, el tipo de enlace que formarán los elementos
X (grupo 1, período 3) e Y (grupo 16, período 3) cuando se combinen y la
fórmula empírica del compuesto formado.

Los valores de electronegatividad en la escala de Pauling de los átomos
de fósforo y de cloro son, respectivamente, 2.1 y 3.0. La molécula
presenta una geometría molecular de pirámide trigonal. Dibuje la
estructura de la molécula y deduzca, a partir de esta estructura y de
los datos suministrados, el carácter polar, o no polar, del .

De los dos conjuntos de números cuánticos ($n$, $l$, $m_l$ y $m_s$) que
se indican, identifique, de forma justificada, el que representa
correctamente un electrón en un átomo:

1.  (3, -2, -1, -1/2);

2.  (3, 2, -1, 1/2)

Escriba las fórmulas semidesarrolladas y nombre tres de los posibles
isómeros constitucionales que tienen la fórmula molecular .
